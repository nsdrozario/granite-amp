...
bool ImKnob::Knob(const char *label, float *value, float speed, float min, float max, const char *format, float radius, ImVec4 color, ImVec4 color_active) {
    ...
    ImVec2 center_position = ImVec2(draw_position.x+radius, draw_position.y+radius +text_padding);
    ...
    *value = std::clamp(*value, min, max);
    ...
    ImGui::InvisibleButton("Knob", ImVec2(radius*2, (radius+text_padding)*2));
    ...
    if (ImGui::IsItemClicked(ImGuiMouseButton_Left)) {
        ...
        *value = std::clamp((1.0f-((angle - max_angle) / (min_angle - max_angle))) * (max-min) + min, min, max);
        interacted = true;
    }
    ...
    if (ImGui::IsItemActive()) {
        ...
        angle = std::clamp(angle, max_angle, min_angle);
        // normalize the angle then scale to the actual values
        *value = std::clamp((1.0f-((angle - max_angle) / (min_angle - max_angle))) * (max-min) + min, min, max);
        interacted = true;
    }
    ...
    draw_list->AddText(ImVec2(draw_position.x+radius*0.25,draw_position.y), IM_COL32(255,255,255,255),label);
    ...
    draw_list->AddText(ImVec2(draw_position.x+radius*0.25,draw_position.y + text_padding + (2.0f * radius)),IM_COL32(255,255,255,255), formatted);
    return interacted;
}